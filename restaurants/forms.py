...
class ReservationForm(ModelForm):
    ...
    def clean(self):
        ...
        if not restaurant.open_past_midnight():
            if cleaned_time > restaurant.last_reservation_time:
                self.add_error('time', 'Reservations are not available at that time.')
        else:
            if (restaurant.last_reservation_time > restaurant.closing_time) and (cleaned_time > restaurant.last_reservation_time):
                self.add_error('time', 'Reservations are not available at that time.')
            elif (restaurant.last_reservation_time < restaurant.closing_time) and (cleaned_time > restaurant.last_reservation_time) and (cleaned_time < restaurant.opening_time):
                self.add_error('time', 'Reservations are not available at that time.')
...
class RestaurantForm(ModelForm):
    ...
    last_reservation_time = TimeField(widget=TimeInput(attrs={'type': 'time' }))
    name = CharField()
    address = CharField()
    phone = CharField()
    ...
    class Meta:
        model = Restaurant
        fields = ['name', 'description', 'address', 'phone', 'capacity', 'opening_time', 'closing_time', 'last_reservation_time', 'image', 'category']
